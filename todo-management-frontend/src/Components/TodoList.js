          ...
          {todos.map(todo => 
            <tr key={todo.id}>
                ...
                <td style={cell}>
                  ...
                  <Button onClick = {() => {
                    deleteTodo(todo.id).then(response => setTodos(todos.filter(t => t.id !== todo.id)))
                  }}sx={{color : "red",backgroundColor : "white",'&:hover' : {color:"white",backgroundColor:"red"}}} variant="contained">Delete</Button>
                  <Button onClick = {() => {
                    markComplete(todo.id).then(() => setTodos((prevTodos) => prevTodos.map((t) => {
                        if(t.id === todo.id){
                            return {
                                ...t,
                                completed : true
                  ...
                  <Button onClick = {() => {
                    markIncomplete(todo.id).then(() => setTodos((prevTodos) => prevTodos.map((t) => {
                        if(t.id === todo.id){
                            return {
                                ...t,
                                completed : false
